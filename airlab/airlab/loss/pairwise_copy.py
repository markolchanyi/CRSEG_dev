...
class NGF(_PairwiseImageLoss):
    ...
    def forward(self, displacement):
        ...
        empty_mask = th.ones_like(self._moving_image.image, dtype=self._dtype, device=self._device)
        value = 0.5 * th.masked_select(-value.pow(2), empty_mask)
...
class CHAN_NGF(_PairwiseImageLoss):
    r""" Implementation of the Normalized Gradient Fields image loss.
    ...
    def forward(self, displacement):
        ...
        # compute squared differences
        MSE_val = (mask - self._fixed_mask.image).pow(2)
        weight = 5.0

        value = 0.5 * th.masked_select(-value.pow(2), mask) + MSE_val*weight
