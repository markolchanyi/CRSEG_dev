...
# Loss base class (standard from PyTorch)
class _PairwiseImageLoss_chan(th.nn.modules.Module):
    def __init__(self, fixed_image, moving_image, f_mask, m_mask, add, mask_weight, fixed_mask=None, moving_mask=None, size_average=True, reduce=True):
        super(_PairwiseImageLoss_chan, self).__init__()
        self._size_average = size_average
        self._reduce = reduce
        self._name = "parent"
    ...
    def GetCurrentMask(self, displacement):
        ...
        """
        # exclude points which are transformed outside the image domain
        maskk = th.zeros_like(self._fixed_image.image, dtype=th.uint8, device=self._device)
        for dim in range(displacement.size()[-1]):
            maskk += displacement[..., dim].gt(1) + displacement[..., dim].lt(-1)

        maskk = maskk == 0

        # and exclude points which are masked by the warped moving and the fixed mask
        if not self._m_mask is None:
            self._warped_moving_maskk = F.grid_sample(self._m_mask.image, displacement)
            self._warped_moving_maskk = self._warped_moving_maskk >= 0.5

            # if either the warped moving mask or the fixed mask is zero take zero,
            # otherwise take the value of mask
            if not self._f_mask is None:
                maskk = th.where(((self._warped_moving_maskk == 0) | (self._f_mask == 0)), th.zeros_like(maskk), maskk)
            else:
                maskk = th.where((self._warped_moving_maskk == 0), th.zeros_like(maskk), maskk)
        return maskk


    def GetCurrentMask_dummy(self, displacement):
        """
        Computes a mask defining if pixels are warped outside the image domain, or if they fall into
        a fixed image mask or a warped moving image mask.
        return (Tensor): maks array
...
# Loss base class (standard from PyTorch)
class _PairwiseImageLoss_2chan(th.nn.modules.Module):
    def __init__(self, fixed_image1, moving_image1, fixed_image2, moving_image2, f_mask, f_submask, m_mask, m_submask, add, mask_weight, submask_weight, channel_weight, fixed_mask=None, moving_mask=None, size_average=True, reduce=True,single_channel=True,no_superstructs=False):
        super(_PairwiseImageLoss_2chan, self).__init__()
        self._size_average = size_average
        self._reduce = reduce
        self._name = "parent"
        ...
        self._f_submask = f_submask
        self._m_submask = m_submask

        self._multiplier = add
        self._mask_weight = mask_weight
        self._submask_weight = submask_weight
    ...
    def GetCurrentMask(self, displacement):
        ...
        """
        # exclude points which are transformed outside the image domain
        maskk = th.zeros_like(self._fixed_image1.image, dtype=th.uint8, device=self._device)
        for dim in range(displacement.size()[-1]):
            maskk += displacement[..., dim].gt(1) + displacement[..., dim].lt(-1)
    ...
    def GetCurrentSubMask(self, displacement):
        """
        Computes a mask defining if pixels are warped outside the image domain, or if they fall into
        a fixed image mask or a warped moving image mask.
        return (Tensor): maks array
        ...
        # and exclude points which are masked by the warped moving and the fixed mask
        if not self._m_submask is None:
            self._warped_moving_maskk = F.grid_sample(self._m_submask.image, displacement)
            self._warped_moving_maskk = self._warped_moving_maskk >= 0.5

            # if either the warped moving mask or the fixed mask is zero take zero,
            # otherwise take the value of mask
            if not self._f_submask is None:
                maskk = th.where(((self._warped_moving_maskk == 0) | (self._f_submask == 0)), th.zeros_like(maskk), maskk)
            else:
                maskk = th.where((self._warped_moving_maskk == 0), th.zeros_like(maskk), maskk)
        return maskk
...
class LCC(_PairwiseImageLoss):
    ...
    def forward(self, displacement):
        ...
        if version.parse(th.__version__) > version.parse("1.1.0"):
            mask = ~mask
        else:
            mask = 1-mask
...
class NGF_CHAN(_PairwiseImageLoss_chan):
    r""" Implementation of the Normalized Gradient Fields image loss.
            Args:
                fixed_image (Image): Fixed image for the registration
                moving_image (Image): Moving image for the registration
                ...
                reduce (bool): Reduce loss function to a single value
    """
    def __init__(self, fixed_image, moving_image, f_mask, m_mask, add=-1.0, mask_weight=2.0, fixed_mask=None, moving_mask=None, epsilon=1e-5,
                 size_average=True,
                 reduce=True):
        super(NGF_CHAN, self).__init__(fixed_image, moving_image, f_mask, m_mask, add, mask_weight, fixed_mask, moving_mask, size_average, reduce)
    ...
    def forward(self, displacement):
        ...
        mask = super(NGF_CHAN, self).GetCurrentMask_dummy(displacement)
        maskk = super(NGF_CHAN, self).GetCurrentMask(displacement)
        ...
        # warp moving mask with displacement field
        warped_m_mask = F.grid_sample(self._m_mask.image, displacement)

        self._f_mask = self._f_mask.to(dtype=self._dtype, device=self._device)
        self._warped_moving_maskk = self._warped_moving_maskk.to(dtype=self._dtype, device=self._device)
        ...
        print("MSE is: ",th.masked_select(MSE_val, maskk).mean())

        value = 0.5 * th.masked_select(-value.pow(2), mask) + self._mask_weight * self._multiplier * th.masked_select(MSE_val, mask)
...
class NGF_2CHAN(_PairwiseImageLoss_2chan):
    r""" Implementation of the Normalized Gradient Fields image loss.
            Args:
                fixed_image (Image): Fixed image for the registration
                moving_image (Image): Moving image for the registration
                ...
                reduce (bool): Reduce loss function to a single value
    """
    def __init__(self, fixed_image1, moving_image1,fixed_image2, moving_image2, f_mask, f_submask, m_mask, m_submask, add=-1.0, mask_weight=2.0, submask_weight=1.0, channel_weight=1.0, fixed_mask=None, moving_mask=None, epsilon=1e-5,
                 size_average=True,
                 reduce=True,
                 single_channel=True,
                 no_superstructs=False):
        super(NGF_2CHAN, self).__init__(fixed_image1, moving_image1, fixed_image2, moving_image2, f_mask, f_submask, m_mask, m_submask, add, mask_weight, submask_weight, channel_weight, fixed_mask, moving_mask, size_average, reduce, single_channel,no_superstructs)

        self._name = "ngf_2chan"
    ...
    def forward(self, displacement):
        ...
        # compute current mask and dummy ones mask
        mask = super(NGF_2CHAN, self).GetCurrentMask_dummy(displacement)
        maskk = super(NGF_2CHAN, self).GetCurrentMask(displacement)
        submask = super(NGF_2CHAN, self).GetCurrentSubMask(displacement)
        ...
        # get distance transforms of masks
        #print(self._m_mask.numpy().shape)
        #[s0, s1, s2] = self._m_mask.numpy().shape
        #m_distance_mask = al.Image(ndimage.distance_transform_edt(self._m_mask.numpy()).astype(np.float32), [s0, s1, s2], self._m_mask.spacing, self._m_mask.origin, dtype=self._dtype)
        #m_distance_submask = al.Image(ndimage.distance_transform_edt(self._m_submask.numpy()).astype(np.float32), [s0, s1, s2], self._m_mask.spacing, self._m_mask.origin, dtype=self._dtype)

        # warp moving mask with displacement field
        warped_m_mask = F.grid_sample(self._m_mask.image, displacement)
        warped_m_submask = F.grid_sample(self._m_submask.image, displacement)

        self._f_mask = self._f_mask.to(dtype=self._dtype, device=self._device)
        self._f_submask = self._f_submask.to(dtype=self._dtype, device=self._device)
