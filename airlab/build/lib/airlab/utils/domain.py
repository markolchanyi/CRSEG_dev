...
def get_joint_domain_images(fixed_image, moving_image, fixed_mask, moving_mask, fixed_mask_extended=None, moving_mask_extended=None, dummy_move=None, default_value=0, interpolator=2, cm_alignment=False, compute_masks=False,align_masks=False):
    """
    ...
    if cm_alignment:
        if align_masks:
            cm_displacement = get_center_of_mass(fixed_mask) - get_center_of_mass(moving_mask)
        else:
        ...
        moving_mask.origin = moving_mask.origin + cm_displacement
        moving_mask_extended.origin = moving_mask_extended.origin + cm_displacement
        if dummy_move is not None:
            print("yes")
            dummy_move.origin = dummy_move.origin + cm_displacement
    ...
    if np.all(fixed_image.origin == moving_image.origin) and\
        ...
        if compute_masks:
            ...
            d_mask = th.ones_like(moving_image.image)
            ...
            d_mask = Image(d_mask, moving_image.size, moving_image.spacing, moving_image.origin)
    ...
    # Mark addition...for real masks with extensions
    f_mask_PM = Image(resampler.Execute(fixed_mask.itk()))
    m_mask_PM = Image(resampler.Execute(moving_mask.itk()))
    
    
    if fixed_mask_extended and moving_mask_extended is not None:
        f_mask_PM_ext = Image(resampler.Execute(fixed_mask_extended.itk()))
        m_mask_PM_ext = Image(resampler.Execute(moving_mask_extended.itk()))
        
    
    if dummy_move is not None:
        dumm = Image(resampler.Execute(dummy_move.itk()))
        dumm.to(dtype=moving_mask.dtype, device=moving_mask.device)
    else:
        dumm = f_mask
    ...
    f_mask_PM.to(dtype=fixed_mask.dtype, device=fixed_mask.device)
    m_mask_PM.to(dtype=moving_mask.dtype, device=moving_mask.device)
    
    f_mask_PM_ext.to(dtype=fixed_mask_extended.dtype, device=fixed_mask_extended.device)
    m_mask_PM_ext.to(dtype=moving_mask_extended.dtype, device=moving_mask_extended.device)
    ...
    return f_image, f_mask, f_mask_PM, m_mask_PM, f_mask_PM_ext, m_mask_PM_ext, m_image, m_mask, cm_displacement, dumm
