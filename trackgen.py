...
for case_path in case_list_full:
    ...
    if not os.path.exists(os.path.join(scratch_dir,"data.nii.gz")):
        ...
        os.system("rsync -av " + os.path.join("/autofs/space/nicc_003/users/olchanyi/data/HCP100/",os.path.basename(case_path),"T1w/Diffusion/data.nii.gz") + " " + scratch_dir)
    if not os.path.exists(os.path.join(scratch_dir,"dwi.bval")):
        os.system("rsync -av " + os.path.join("/autofs/space/nicc_003/users/olchanyi/data/HCP100/",os.path.basename(case_path),"T1w/Diffusion/bvals") + " " + os.path.join(scratch_dir,"dwi.bval"))
    if not os.path.exists(os.path.join(scratch_dir,"dwi.bvec")):
        os.system("rsync -av " + os.path.join("/autofs/space/nicc_003/users/olchanyi/data/HCP100/",os.path.basename(case_path),"T1w/Diffusion/bvecs") + " " + os.path.join(scratch_dir,"dwi.bvec"))


    samseg_path = os.path.join(case_path,"samseg_labels","")
    if not os.path.exists(samseg_path + "seg.mgz"):
        os.system("run_samseg -i " + os.path.join("/autofs/space/nicc_003/users/olchanyi/data/HCP100/",os.path.basename(case_path),"T1w/Diffusion/lowb.nii") + " -o " + samseg_path + " --threads 8")
    ...
    if not os.path.exists(os.path.join(scratch_dir,"brainstem.mif")):
    ...
    if not os.path.exists(os.path.join(scratch_dir,"tracts_thal.tck")):
        ...
        os.system("tckgen -algorithm iFOD2 -angle 50 -select 100000 -seed_image " + os.path.join(scratch_dir,"thal.mif") + " -include " + os.path.join(scratch_dir,"brainstem.mif") + " " + os.path.join(scratch_dir,"wmfod_norm.mif") + " " + os.path.join(scratch_dir,"tracts_thal.tck") + " -force -nthreads 8")
    if not os.path.exists(os.path.join(scratch_dir,"tracts_DC.tck")):
        print("starting tracking on DC.mif")
        os.system("tckgen -algorithm iFOD2 -angle 50 -select 100000 -seed_image " + os.path.join(scratch_dir,"DC.mif") + " -include " + os.path.join(scratch_dir,"brainstem.mif") + " " + os.path.join(scratch_dir,"wmfod_norm.mif") + " " + os.path.join(scratch_dir,"tracts_DC.tck") + " -force -nthreads 8")
    if not os.path.exists(os.path.join(scratch_dir,"tracts_CB.tck")):
        print("starting tracking on CB.mif")
        os.system("tckgen -algorithm iFOD2 -angle 50 -select 50000 -seed_image " + os.path.join(scratch_dir,"CB.mif") + " -include " + os.path.join(scratch_dir,"DC.mif") + " " + os.path.join(scratch_dir,"wmfod_norm.mif") + " " + os.path.join(scratch_dir,"tracts_CB.tck") + " -force -nthreads 8")
